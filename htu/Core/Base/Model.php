...
class Model
{
    ...
    {
        $data = array();
        $result = $this->connection->query("SELECT * FROM $this->table  ORDER BY selling_price DESC ");
    ...
    public function last_id($last_insert_id)
    {
        $stmt=$this->connection->prepare("SELECT * FROM $this->table WHERE id=LAST_INSERT_ID()");
         // prepare the sql statement
        $stmt->bind_param('i', $last_insert_id); // bind the params per data type (https://www.php.net/manual/en/mysqli-stmt.bind-param.php)
        $stmt->execute(); // execute the statement on the DB
        $result = $stmt->get_result(); // get the result of the execution
        $stmt->close();
        // $result = $this->connection->query("SELECT * FROM $this->table WHERE id=$id");
